...
export default function ParentDashboard() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
      {/* ── WELCOME HEADER ── */}
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-10 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        {data?.children.map((child, i) => (
          <div key={i} className="bg-white rounded-[2rem] p-8 shadow-xl shadow-gray-100/50 border border-gray-50 flex flex-col md:flex-row gap-8 hover:border-indigo-100 transition-all duration-300 group">
             {/* LEFT: Profile & Basic Info */}
             <div className="flex flex-col items-center gap-4 text-center">
                <div className="w-24 h-24 bg-indigo-50 rounded-[2rem] flex items-center justify-center text-indigo-600 text-3xl font-bold shadow-inner border border-indigo-100">
                  {child.name[0]}
                </div>
                <div className="space-y-1">
                   <h3 className="text-xl font-bold text-gray-800">{child.name}</h3>
                   <p className="text-xs font-bold text-indigo-500 uppercase tracking-widest">{child.class} - {child.section}</p>
                </div>
                <button 
                  className="mt-2 w-full px-4 py-2 bg-gray-50 hover:bg-indigo-600 hover:text-white text-gray-500 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 group/btn"
                >
             ...
             <div className="flex-1 grid grid-cols-2 gap-4">
                ...
                <div className={`p-5 rounded-3xl border space-y-2 ${child.isFeeOverdue ? 'bg-rose-50 border-rose-100' : 'bg-indigo-50/50 border-indigo-100/30'}`}>
                   <div className="flex items-center justify-between">
                      <p className={`text-[10px] font-bold uppercase tracking-wider ${child.isFeeOverdue ? 'text-rose-600' : 'text-indigo-600'}`}>Fee Status</p>
                      {child.isFeeOverdue ? <AlertCircle size={16} className="text-rose-500" /> : <Wallet size={16} className="text-indigo-500" />}
                   </div>
                   <p className={`text-xl font-black uppercase ${child.isFeeOverdue ? 'text-rose-700' : 'text-indigo-700'}`}>{child.feeStatus}</p>
                   <p className={`text-[9px] font-medium italic ${child.isFeeOverdue ? 'text-rose-400' : 'text-indigo-400'}`}>
